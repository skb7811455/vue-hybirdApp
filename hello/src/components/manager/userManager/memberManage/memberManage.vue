<template>
    ...
    <ul>
      <li v-for="(member,index) in memberList" @touchend="touchEnd(index)">
        <div class="user-box" @touchmove="touchMove(index)" >  
          <div class="name-box" @touchstart="startTouch()" ref="namebox" style="0px">        
          <span class="username">{{member.name}}</span>
          </div>
          <div class="hide-box" ref="hidebox" style="right:-195px">
            <div class="pop-text remind" ref="popbox" @click.stop="back(index)"><span class="text">提醒</span></div>
            <div class="pop-text warn" ref="popbox2" @click.stop="back(index)"><span class="text">警告</span></div>         
    ...
    ,
      ...
      touchEnd:function(index){
        if(this.distance<44){
          ...
          this.$refs.namebox[index].style.left=-264+"px";
          this.$refs.popbox[index].style.width=88+"px";
          if(!this.showapply){
            this.$refs.popbox2[index].style.width=88+"px";
          }                
